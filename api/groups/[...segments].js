...
import { PROVIDERS, listProviders, getEmoji } from '../../lib/sports/index.js';
...
export default requireAuth(async function handler(req, res) {
  // NB: /api/groups (0 segment) est géré par api/groups/index.js — ce fichier
  // ne reçoit que des requêtes avec au moins 1 segment (catch-all obligatoire).
...
async function competitionsRoot(req, res, groupId) {
  if (req.method === 'GET') {
    ...
    const shaped = competitions.map((c) => ({ ...c, emoji: getEmoji(c.provider) }));
    return res.status(200).json({ competitions: shaped, providers: listProviders() });
  }
