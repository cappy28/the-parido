...
document.querySelectorAll('.tab-btn[data-tab]').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    void panel.offsetWidth; // force le navigateur à relire le style avant de rejouer l'anim
...
// Fait "grandir" les barres de cotes depuis 0 au lieu de les afficher déjà
// pleines — signature visuelle du tableau (comme un trait de craie qui se trace).
function animateBars(scope) {
  const bars = scope.querySelectorAll('.bar-fill[data-target]');
...
function renderBetCard(bet, idx) {
  ...
  const optionsHTML = bet.options.map((o) => {
    ...
    return `
      <div class="option-row ${won ? 'won' : ''} ${lost ? 'lost' : ''}">
        ...
        <div class="bar-track"><div class="bar-fill" style="width:0%" data-target="${share}"></div></div>
        ${bet.status === 'OPEN' ? `
          <div class="wager-row">
  ...
  return `
    <div class="board-card" style="--i:${idx}">
...
async function loadLeaderboard() {
  try {
    ...
    document.getElementById('leaderboardList').innerHTML = leaderboard.map((row, idx) => `
...
async function loadMembers() {
  try {
    ...
    document.getElementById('memberList').innerHTML = members.map((m, idx) => `
      <div class="member-row" style="--i:${idx}">
  ...
  try {
    ...
    if (items.length === 0) {
      ...
      grid.innerHTML = items.map((it, idx) => {
        ...
        return `
        <div class="shop-card ${!it.active ? 'inactive' : ''}" style="--i:${idx}">
          ${visual}
...
function applyProviderHints() {
  const p = providerList.find((x) => x.key === document.getElementById('compProvider').value);
  if (!p) return;
  document.getElementById('compIdHint').textContent = p.competitionIdHint || '';
  document.getElementById('compSeasonHint').textContent = p.seasonHint || '';
  const compId = document.getElementById('compId');
  if (p.defaultCompetitionId && !compId.value) compId.value = p.defaultCompetitionId;
  document.getElementById('compSeason').required = !!p.seasonRequired;
}

async function loadCompetitions() {
  document.getElementById('adminAutoForm').classList.toggle('hidden', !isAdmin);
  try {
    const { competitions, providers } = await api(`/groups/${groupId}/competitions`);
    providerList = providers;

    const select = document.getElementById('compProvider');
    if (select.options.length === 0) {
      select.innerHTML = providers.map((p) => `<option value="${p.key}">${p.emoji} ${escapeHTML(p.label)}</option>`).join('');
      select.addEventListener('change', applyProviderHints);
      applyProviderHints();
    }
